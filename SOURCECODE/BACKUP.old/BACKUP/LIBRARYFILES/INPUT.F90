	...
	!INPUTS INDEX OF METADYNAMICS COLLECTIVE VARIABLES AND CHECKS FOR ERROR.
		...
		DO ITER1 = 1,NMETACOLVAR
			IF ((IMETACOLVAR(ITER1) .LT. 1) .OR. (IMETACOLVAR(ITER1) .GT. NMETACOLVAR)) THEN
	...
	!READS TEMPERATURE AND APLHA AND CHECKS FOR ERROR.
		...
		IF (WTALPHA -1.0D0  .LT. 0.0001D0) THEN
			WELLTEMP = .TRUE.
			WTDT = TEMP / (1.0D0 - WTALPHA)
		ENDIF
	...
	IF (MPIRANK .EQ. 0) THEN
		...
		DO ITER1 = 1,NDIM - 1
        	...
        	ENDDO
        	WRITE (COMMENT,'(F8.5,A)') GRIDMAX(ITER1), ']'
		WRITE(1,'(A,T50,A,A)') 'RANGE', ':  ', ADJUSTL (TRIM (TRIM (GRID) // COMMENT))
		CLOSE(1)
	ENDIF
	RETURN
	END SUBROUTINE
	...
	!INPUTS METADYNAMICS PARAMETERS, CALCULATES RANGE OF INTEGRATION.
        ...
        !FINDS THE STARTING POINT  OF INTEGRATION.
        				S(1:NMETACOLVAR) = CENTER(1:NMETACOLVAR,ITER1) - 5.0D0 * WIDTH(ITER1)
        				CALL GETI
        				IMIN(1:NMETACOLVAR,ITER2) = MAX (0,I(1:NMETACOLVAR))
        !FINDS THE END POINT  OF INTEGRATION.
        				S(1:NMETACOLVAR) = CENTER(1:NMETACOLVAR,ITER1) + 5.0D0 * WIDTH(ITER1)
        				CALL GETI
        				IMAX(1:NMETACOLVAR,ITER2) = MIN (NBIN(1:NMETACOLVAR),I(1:NMETACOLVAR))
        			ENDIF
        		ENDDO
        		CLOSE(1)
        ...
        !FINDS THE STARTING POINT  OF INTEGRATION.
        				S(1:NMETACOLVAR) = CENTER(1:NMETACOLVAR,ITER1) - 5.0D0 * WIDTH(ITER1)
        				CALL GETI
        				IMIN(1:NMETACOLVAR,ITER1) = MAX (0,I(1:NMETACOLVAR))
        !FINDS THE END POINT  OF INTEGRATION.
        				S(1:NMETACOLVAR) = CENTER(1:NMETACOLVAR,ITER1) + 5.0D0 * WIDTH(ITER1)
        				CALL GETI
	...
	!INPUT PROCEDURE FOR TOYCODE-1.0.
        ...
        !READS HILL PARAMETERS.
        		DO ITER1 = 1,NHILL
        			READ (1,*,IOSTAT = IOSTATUS1) TIME(ITER1), (CENTER(ITER2,ITER1), ITER2 = 1,NMETACOLVAR),  HEIGHT(ITER1), WIDTH(ITER1)
        !CHECKS FOR EOF OR CORRUPT INPUT.
        			IF(IOSTATUS1 .GT. 0) THEN
        				CALL PRINT_ERROR ('CORRUPT VALUES READ FROM HILLFILES/HILLS.')
        				ERRORSTAT = .TRUE.
        ...
        !FINDS THE END POINT  OF INTEGRATION.
        				S(1:NMETACOLVAR) = CENTER(1:NMETACOLVAR,ITER1) + 5.0D0 * WIDTH(ITER1)
        				CALL GETI
        				IMAX(1:NMETACOLVAR,ITER1) = MIN (NBIN(1:NMETACOLVAR),I(1:NMETACOLVAR))
        			ENDIF
        		ENDDO
        		CLOSE(1)
		ENDIF
